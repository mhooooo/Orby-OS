...
export default function ImportCoursesPage() {
  ...
  const handleFileChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      for (let i = 1; i < Math.min(lines.length, 6); i++) {
        ...
        const row: ParsedRow = { course_name: '', rate_type: '', net_rate: '' };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Header */}
      <div>
        <h2 className="text-2xl font-bold text-white">Import Course Rates</h2>
        <p className="text-gray-400 mt-1">Upload a CSV file with course rate data</p>
      </div>
        ...
        <div className="mt-6">
          ...
          <div className="bg-white/5 rounded-xl p-4 overflow-x-auto">
            <code className="text-xs text-gray-300">
              course_name,rate_type,net_rate,rack_rate,day_type,season<br/>
              Thai Country Club,green_fee,4500,6000,weekday,high<br/>
              Thai Country Club,green_fee,5500,7000,weekend,high
            </code>
          </div>
        </div>
      </div>
